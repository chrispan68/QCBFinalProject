...
def scoreAll():
	df = pd.DataFrame()
	folders = ['long_background', 'long_motif', 'plasmodium_falciparum', 'random_motif', 'short_motif', 'tandem_repeat']
	for variable in folders:
		...
		with open(variable + '/GAME.json') as f:
			game = pd.read_json(f)
		if(variable != 'random_motif' and variable != 'tandem_repeat'):
